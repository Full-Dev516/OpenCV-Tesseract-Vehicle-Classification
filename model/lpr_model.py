...
def crnn_predict(crnn, img, transformer, num_top_results=5):
    ...
    for (seq, conf) in seq_conf:
        ...
        sim_pred = converter.decode(seq_tensor.data, seq_tensor_size.data, raw=False).upper()
    ...
    def predict(self, img_path):
        ...
        if len(self.crop_info) == 2:       
            ...
            for single_crop_info in self.crop_info:
                ...
                pred_conf_multi.append(crnn_predict(self.crnn, cropped, self.transformer, self.num_top_results))
        ...
        # if detected text regions are not exactly 2, assume it's single line with noises
            ...
            pred_conf = crnn_predict(self.crnn, self.image, self.transformer, self.num_top_results)
            return (pred_conf)
